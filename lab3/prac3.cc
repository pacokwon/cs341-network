...
/*
 * https://www.nsnam.org/docs/tutorial/html/conceptual-overview.html#a-first-ns-3-script really useful material
 */
...
Ptr<PacketSink> sink3;     /* Pointer to the packet sink application */
...
uint64_t lastTotalRx3(0);  /* The value of the last total received bytes */

void ReceivedBytes1(Ptr<OutputStreamWrapper> stream)
{
  Time now = Simulator::Now();                                         /* Return the simulator's virtual time. */
  /*
     stream's text structure:
     {current time in seconds}\t{throughput in Mbps}\t{Received bytes in sink1}
  */
...
void ReceivedBytes2(Ptr<OutputStreamWrapper> stream)
{
  ...
  /*
     stream's text structure:
     {current time in seconds}\t{throughput in Mbps}\t{Received bytes in sink2}
  */
  *stream->GetStream() << now.GetSeconds() << "\t"
                       << (sink2->GetTotalRx() - lastTotalRx2) * 8 / 0.1 * 1e-6 << "\t" // throughput in Mbps
                       << sink2->GetTotalRx() << "\t" << std::endl; // received bytes in sink1
...
void ReceivedBytes3(Ptr<OutputStreamWrapper> stream)
{
  ...
  *stream->GetStream() << now.GetSeconds() << "\t"
                       ...
                       << sink3->GetTotalRx() << "\t" << std::endl; // received bytes in sink1
...
int main(int argc, char *argv[])
{
  ...
  internet.Install(c); // install internet stacks in the nodes that are in the container
  ...
  d1d0.Get(0)->SetAttribute("ReceiveErrorModel", PointerValue(em));

  p2p.SetDeviceAttribute("DataRate", StringValue("2Mbps"));
  p2p.SetChannelAttribute("Delay", StringValue("10ms"));
  NetDeviceContainer d2d1 = p2p.Install(n2n1); // link between node 1 and node 2

  p2p.SetDeviceAttribute("DataRate", StringValue("5Mbps"));
  p2p.SetChannelAttribute("Delay", StringValue("10ms"));
  NetDeviceContainer d3d1 = p2p.Install(n3n1); // link between node 1 and node 3

  p2p.SetDeviceAttribute("DataRate", StringValue("1Mbps"));
  p2p.SetChannelAttribute("Delay", StringValue("10ms"));
  NetDeviceContainer d4d1 = p2p.Install(n4n1); // link between node 1 and node 4

  // Later, we add IP addresses.
  NS_LOG_INFO("Assign IP Addresses.");
  Ipv4AddressHelper ipv4; // IP address
  ipv4.SetBase("10.1.1.0", "255.255.255.0");        // define network allocation scheme
  Ipv4InterfaceContainer i1i0 = ipv4.Assign(d1d0);  // assign address according to scheme -> will get 10.1.1.1

  ipv4.SetBase("10.1.2.0", "255.255.255.0");        // define network allocation scheme
  Ipv4InterfaceContainer i2i1 = ipv4.Assign(d2d1);  // assign address according to scheme -> will get 10.1.2.1
